...
<head>
  ...
  <title>Ginjo Administration Admin Dashboard</title>
  <!-- Bootstrap CSS -->
...
<body>
  <div class="d-flex">
    ...
    <div class="sidebar d-flex flex-column p-3" id="adminSidebar">
      ...
      <a href="manage_request.php" target="contentFrame" class="btn btn-link text-start sidebar-link">
      ...
      <a href="Manage_residents.php" target="contentFrame" class="btn btn-link text-start sidebar-link">
        <i class="bi bi-people-fill"></i> Manage Residents
      </a>
      <a href="manage_users.php" target="contentFrame" class="btn btn-link text-start sidebar-link">
        <i class="bi bi-person-gear"></i> Manage Users
      </a>
      <a href="Search_Residents.php" target="contentFrame" class="btn btn-link text-start sidebar-link">
        <i class="bi bi-search"></i> Search Residents
      </a>
      <a href="Applications.php" target="contentFrame" class="btn btn-link text-start sidebar-link">
        <i class="bi bi-file-earmark-text"></i> Applications
      </a>
      <a href="../Staff_Dashboard/verify_cert.php" target="contentFrame" class="btn btn-link text-start sidebar-link">
        <i class="bi bi-patch-check"></i> Verified Certificates
      </a>
      <a href="Generate_report.php" target="contentFrame" class="btn btn-link text-start sidebar-link">
        <i class="bi bi-bar-chart-line-fill"></i> Generate Reports
      </a>
      <a href="settings.php" target="contentFrame" class="btn btn-link text-start sidebar-link">
    ...
    <div class="w-100">
      <div class="bg-primary text-white p-3 fixed-top d-flex justify-content-between align-items-center" style="margin-left: 250px;">
        <h1 class="h4 mb-0">Welcome to Admin Dashboard</h1>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function setActiveSidebar(href) {
        sidebarLinks.forEach(link => {
          ...
          // Add active if href matches
          if (link.getAttribute('href') && href && link.getAttribute('href').toLowerCase() === href.toLowerCase()) {
      ...
      if (iframe) {
        iframe.addEventListener('load', function() {
          try {
            // Get the loaded page's filename
